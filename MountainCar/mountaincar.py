...
def get_location(screen_width):
    ...
    return env.state[0]#int(env.state[0] * scale + screen_width / 2.0)
...
if __name__=='__main__':
        ...
        policy_net.load_state_dict(torch.load('models/MountainCar-50-epochs.model'), strict=False)

        if len(sys.argv) > 1:
            num_episodes = int(sys.argv[1])
        elif len(sys.argv) > 2:
            policy_net.load(sys.argv[2])
        else:
            num_episodes = 50 

        episode_durations = []  
        #policy_net.load_state_dict(torch.load('models/100000-epochs.model'))
